...
market_cap_count = Counter(
    {'BitMEX': 1115820000, 'Bitfinex': 893217509, 'OKEx': 852615381, 'Binance': 395220130, 'GDAX': 335818600,
     ...
     'Cryptopia': 848882, 'Bitstamp': 641082, 'Coinut': 380576, 'OkCoin Intl.': 311313, 'Tidex': 92987,
...
def fetch_reviews(exchange_names):
    with GoogleSearch() as searcher:
        for name in exchange_names:
            query = "%s reviews" % name
            yield name, searcher.search(query).results


# TODO: count the domains
def count_domains(exchange_names):
    domain_count = Counter()
    for name, results in fetch_reviews(exchange_names):
        domain_count.update(set(urlparse(result.url).netloc for result in results))
        parsers.parse_reviews(results)
    print (domain_count)



if __name__ == "__main__":
    count_domains(name for name in coin_count)
